...
export class OpenMeteoProvider implements WeatherProvider {
  ...
  async searchIndianCities(query: string): Promise<IndianCity[]> {
    ...
    return (body.results ?? [])
      .filter((result) => result.country_code === 'IN')
      .map((result) => ({
        id: String(result.id), name: String(result.name), admin1: result.admin1 ? String(result.admin1) : undefined,
        countryCode: 'IN' as const, latitude: Number(result.latitude), longitude: Number(result.longitude),
      }));
  }
  ...
  async getDailyWeather(city: IndianCity): Promise<WeatherSnapshot> {
    ...
    try {
      const params = new URLSearchParams({
        latitude: String(city.latitude), longitude: String(city.longitude), timezone: 'auto', forecast_days: '1',
        daily: 'temperature_2m_max,temperature_2m_min,precipitation_sum,precipitation_probability_max,sunshine_duration',
