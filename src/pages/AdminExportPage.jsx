  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        error('Failed to load products for export');
      } finally {
        setLoading(false);
      }
    };
      ...
      <div className="flex items-center justify-between mb-4">
        <div>
          ...
          <p className="text-sm text-[var(--text-secondary)]">
            Generate a printable PDF catalog of all currently available products.
          </p>
        </div>
      ...
      <div className="card p-5 flex flex-col gap-3">
        <p className="text-sm text-[var(--text-secondary)]">
          ...
          <span className="font-semibold text-[var(--text-primary)]">
            {loading ? '…' : products.filter(p => p.available).length}
          </span>
        </p>
        <button
          type="button"
        ...
        >
          {loading ? 'Loading products…' : 'Export available as PDF'}
        </button>
      </div>
    </div>
  );
