...
function normalizePermission(display: string): ReminderPermission {
...
export class CapacitorNotificationDriver implements NotificationDriver {
  ...
  async schedule(reminder: ScheduledReminder) {
    await LocalNotifications.schedule({ notifications: [{
      id: reminder.id, title: reminder.title, body: reminder.body, schedule: { at: reminder.at }, extra: { taskId: reminder.taskId },
    }] });
  }
