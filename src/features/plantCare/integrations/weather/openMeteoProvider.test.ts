...
const apiResponse = {
  daily: { time: ['2026-07-14'], temperature_2m_max: [31], temperature_2m_min: [24], precipitation_sum: [12], precipitation_probability_max: [75], sunshine_duration: [14400] },
  hourly: { relative_humidity_2m: [80, 84, 76] },
};

describe('OpenMeteoProvider', () => {
  it('normalizes provider data into the care weather contract', () => {
    expect(normalizeForecast(apiResponse, new Date('2026-07-14T08:00:00.000Z'))).toEqual(expect.objectContaining({ availability: 'live', source: 'open-meteo', precipitationMm: 12, humidityPercent: 80, fetchedAt: '2026-07-14T08:00:00.000Z' }));
  });
  ...
  it('returns seasonal fallback when the provider is unavailable', async () => {
    ...
    await expect(provider.getDailyWeather({ id: 'kochi', name: 'Kochi', countryCode: 'IN', latitude: 9.93, longitude: 76.26 })).resolves.toEqual(expect.objectContaining({ availability: 'seasonal-fallback' }));
  });
});
