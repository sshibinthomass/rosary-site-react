import{r as c,j as t,N as _}from"./vendor-react--rIqVvME.js";import{getAllProducts as $}from"./productService-DktgS5PX.js";import{a as N,C as b,v as z}from"./index-DRCJmyos.js";import"./vendor-firebase-firestore-BbvuCeMf.js";import"./vendor-firebase-app-DU3ujpbt.js";import"./vendor-firebase-storage-B721vxUT.js";import"./firebase-3OUo4N3O.js";import"./firebaseOptions-BhGK3f0q.js";function S(){const{error:n}=N(),[d,u]=c.useState([]),[l,v]=c.useState(!0);c.useEffect(()=>{(async()=>{try{const a=await $();u(a)}catch{n("Failed to load products for export")}finally{v(!1)}})()},[n]);const w=()=>{const o=d.filter(e=>e.available);if(!o.length){n("No available products to export");return}const a=window.open("","_blank");if(!a){n("Please allow pop-ups to export the PDF");return}const y=new Date().toLocaleString("en-IN"),p=window.location.origin,f=o.map(e=>{const s=e.title||e.commonName||e.name||"",h=s.length>20?s.slice(0,20):s,r=e.imageUrl||"/placeholder-plant.jpg";let i;r.startsWith("http://")||r.startsWith("https://")?i=r:r.startsWith("public/")?i=p+"/"+r.slice(7):i=p+(r.startsWith("/")?"":"/")+r;const P=(e.salesPrice||e.price||"").toString(),x=e.originalPrice?e.originalPrice.toString():"",j=`${p}/plant/${e.id}`;return`
      ...
      `}),m=12,g=[];for(let e=0;e<f.length;e+=m){const s=f.slice(e,e+m).join("");g.push(s)}const k=`
      <!doctype html>
      <html>
        <head>
          <meta charset="utf-8" />
